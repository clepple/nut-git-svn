...
void print_usage() {
	...
	printf("-c, --current_config_dir  the directory where are the current old way configuration\n");
	printf("                          files. Default is the default base configuration\n");
	printf("-s, --single              save the configuration in a single file\n");
	printf("-m, --mode                as it is difficult to find the mode of the\n");
	printf("                          old configuration, you can put it here. Valid\n");
...
{
	upslogx(LOG_ERR, "Fatal error in parseconf: %s", errmsg);
}
...
void do_upsconf_args(char *upsname, char *var, char *val)
{
	...
	/* "global" stuff */
	if (!upsname) {
		s = xmalloc(sizeof(char) * (strlen(var) + 9));
		sprintf(s, "nut.ups.%s", var);
	...
	set_variable(s, var, string_type);
	free(s);
	free(upsname);
...
void find_mode(int ups, int users, int upsd, int upsmon) {
	t_enum_string enum_string;
	...
	if (!upsd && !upsmon) {
		if (!quiet) {
			printf("No upsd nor upsmon configuration found.\n=)>Assuming you are in pm mode\n");
		}
		set_mode(pm);
		return;
	}
	if (!upsd && upsmon) {
		if (!quiet) {
			printf("No upsd but upsmon onfiguration found\n=)>Assuming you are in network client mode\n");
		}
		set_mode(net_client);
		return;
	}
	
	enum_string = get_ups_list();
	if (enum_string !=0 && enum_string->next_value != 0) {
		if (!quiet) {
			printf("upsd.conf found and more than one ups declaration found\n");
			printf("=)>Assuming you are in network server mode\n");
		}
		set_mode(net_server);
		return;
	}
	...
	if (enum_string !=0) {
		if (!quiet) {
			printf("Only one UPS declaration found.\n");
			printf("You are perhaps in network server mode, but I have no clues\n");
			printf("Manually change the configuration if I mistake");
			printf("=)>Assuming you are in standalone mode");
		}
		set_mode(standalone);
		return;
	}
	if (!quiet) {
		printf("I didn't achieve to find your mode. I put your mode to none");
	}
	set_mode(no_mode);
}
...
int main (int argc, char** argv) {
	...
	find_mode(ups, users, upsd, upsmon);
	...
	comm_file = xmalloc(sizeof(char) * (strlen(current_dir) + strlen(s) + 30));
	sprintf(comm_file, "%s/comments/conf.comm.%s", current_dir, s);
	...
	if (test == 0) {
		...
		test = fopen(comm_file, "r");
		if (test == 0) {
			sprintf(comm_file, "%s/comments/conf.comm.C", current_dir);
			test = fopen(comm_file, "r");
			if (test == 0) {
				free(comm_file);
				comm_file = 0;

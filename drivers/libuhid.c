/*!
 * @file libuhid.c
 * @brief HID Library - *BSD uhid backend (minimal interface)
 * Based on libshut.c
 *
 * @author Copyright (C) 2006 Arnaud Quette <aquette.dev@gmail.com>
 *         Copyright (C) 2007 Charles Lepple <clepple@gmail.com>
 *
...
#define UHID_DRIVER_NAME	"uhid communication driver"
#define UHID_DRIVER_VERSION	"0.1"
#define MAX_TRY 4
...
int libuhid_get_report(shut_dev_handle_t *devp, int ReportId,
		       unsigned char *raw_buf, int ReportSize );
int shut_set_report(shut_dev_handle_t *sdev, int id, unsigned char *pkt, int reportlen);
int libuhid_get_interrupt(shut_dev_handle_t *devp, unsigned char *buf,
...
int libuhid_open(shut_dev_handle_t **sdevp, HIDDevice_t *curDevice,
		 HIDDeviceMatcher_t *matcher, unsigned char *ReportDesc, int mode)
{
	...
	upsdebugx(2, "libuhid_open: using port %s", devp->device_path);
	...
	if (ioctl(devp->upsfd, USB_GET_REPORT_ID, &res) < 0) {
		perror("USB_GET_REPORT_ID");
		if(errno == EINVAL) {
			upsdebugx(1, "Is this a uhid node?");
		}
		return -1;
	} else {
...
void libuhid_close(shut_dev_handle_t *sdev)
{
...
{
#if 0
	upsdebugx(4, "Entering libuhid_get_report");
...
int libuhid_set_report(shut_dev_handle_t *devp, int ReportId,
		       unsigned char *raw_buf, int ReportSize )
{
	int ret = 0;
...
int libuhid_get_string(shut_dev_handle_t *devp, int StringIdx, char *buf)
{
	int ret = -1;
...
communication_subdriver_t uhid_subdriver = {
	UHID_DRIVER_NAME,
	UHID_DRIVER_VERSION,
	libuhid_open,
	libuhid_close,

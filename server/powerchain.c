 ...
 * - process multi parent / child (ie, child with several parents, as for STS,
 *   APP, PDU as parent)
...
static powerchain_t *powerchain_create(powerlink_t *root_link)
{
	...
	if (pchains == NULL) {
		...
		/* Append to the existing list of Powerchains */
		pchains->next = temp;
	}
...
/* Copy a Powerchain, as a whole or up to (including) 'node' */
static powerchain_t *powerchain_copy(powerlink_t *pchain, const char* node)
{
	powerchain_t	*temp;
	powerchain_t	*pchains = powerchains;
	powerlink_t		*root_link, *tmp_link;

	if (pchain == NULL) {
		return;
	}

	upsdebugx(3, "powerchain_copy: copying %s", pchain->name);

	root_link = powerlink_create(pchain->name, pchain->parent, pchain->child);
	powerchain_create(root_link);
	
	pchain = pchain->child;
	while (pchain) {
		tmp_link = powerlink_create(pchain->name, pchain->parent, pchain->child);
		//powerchain_add(const char *link_name, const char *parent)
		
		if (!strncmp(pchain->name, node)) {
...
powerlink_t *powerchain_get_next(const char *link_name, powerchain_t *pcref)
{
	...
	while (ptmp) {
		...
		/* Loop on Powerlinks */
		while (plink) {
			upsdebugx(1, "powerchain_get: checking against %s", plink->name);
